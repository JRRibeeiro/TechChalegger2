...
func (a *App) runEvaluationLogic(info *CombinedFlagInfo, userID string) bool {
	log.Printf("DEBUG flag=%+v", info.Flag)
	log.Printf("DEBUG rule=%+v", info.Rule)

	if info.Flag == nil {
		log.Println("DEBUG: flag nil")
		return false
	}

	if !info.Flag.IsEnabled {
		log.Println("DEBUG: flag desabilitada")
		return false
	}

	if info.Rule == nil {
		log.Println("DEBUG: regra nil → TRUE")
		return true
	}
	rule := info.Rule.Rules
	log.Printf("DEBUG rule.type=%s rule.value=%v (%T)", rule.Type, rule.Value, rule.Value)

	if rule.Type != "PERCENTAGE" {
		log.Println("DEBUG: tipo de regra diferente de PERCENTAGE")
		return false
	}

	percentage, ok := rule.Value.(float64)
	if !ok {
		log.Println("DEBUG: value não é float64")
		return false
	}

	bucket := getDeterministicBucket(userID + info.Flag.Name)
	log.Printf("DEBUG bucket=%d percentage=%f", bucket, percentage)
